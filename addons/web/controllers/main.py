...
class Database(http.Controller):
    ...
    def create(self, master_pwd, name, lang, password, **post):
        _logger.info("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")
        _logger.info(post)
        _logger.info(master_pwd)
        _logger.info(name)
    ...
    @http.route('/script/database/create/api', type='json', auth="none", methods=['POST'], csrf=False)
    def dbcreateScript(self, **post):
        _logger.info("jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj")
        _logger.info(post)
        master_pwd = post.get('master_pwd')
        name = post.get('name')
        ...
        try:
            _logger.info("|||||||||||||trying to create database from Script run|||||||||")
            if not re.match(DBNAME_PATTERN, name):
                raise Exception(_('Invalid database name. Only alphanumerical characters, underscore, hyphen and dot are allowed.'))
            # country code could be = "False" which is actually True in python
            country_code = post.get('country_code') or False
